...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// 调用BTreePageId的pgcateg，获取该数据页的类型
		{
			//该数据页是叶子节点，递归结束，直接返回
			//调用BTreeFile的getPage函数，根据本函数传入的参数——TransactionId、脏页列表、BTreePageId、访问权限
		...
		//如果field是空直接找到最左的叶子页面即可
		if (f == null)
			//参数pid处调用最左的孩子（iter是迭代器，iter.next迭代第一个entry，第一个entry的leftchild就是最左孩子）
			return findLeafPage(tid, dirtypages, iter.next().getLeftChild(), perm, f);
		BTreeEntry entry = null;

		//循环遍历所有entry，找到第一个大于（或等于）field的entry，然后递归其左孩子
		while (iter.hasNext()) {
			entry = iter.next();
			// Op.GREATER_THAN_OR_EQ是 大于等于 的意思；entry对应BtreeEntry.java，getkey是这个entry存的field
			if (entry.getKey().compare(Op.GREATER_THAN_OR_EQ, f))
				return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
		}
		// 如果到了最后一个页面，并且最后一个页面都不大于等于field，则递归其右孩子
	...
	private BTreeLeafPage ReversefindLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid,
		...
		if (f == null) {
			// f = null的时候，返回最右节点
			return findLeafPage(tid, dirtypages, iter.next().getRightChild(), perm, f);
		}

		BTreeEntry entry = null;
		//循环遍历所有entry，找到第一个大小于（或等于）field的entry，然后递归其右孩子
		while(iter.hasNext()){
			entry = iter.next();
		...
		// 如果到了最后一个页面，并且最后一个页面都不小于等于field，则递归其左孩子
		return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);

	}

	/*
	 *reverse find leaf page
	 */
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		/**
		 * 拆分叶页,根据需要递归拆分父节点以容纳新entry。
		 * 新entry应该有一个键，该键与右侧page中第一个元组的键字段相匹配（该键是“复制的”），它的子指针指向拆分后的两个叶页
		 * 还需要更新同级指针和父指针
		 *
		 *通过在现有页面右侧添加一个新页面并将一半元组移动到新页面来拆分叶页面。
		 * 将中间键向上复制到父页面中，并根据需要递归拆分父页面以容纳新条目。
		 * getParentWithEmtpySlots（）在这里很有用。
		 * 不要忘记更新所有受影响的叶页的同级指针。返回应该插入具有给定键字段的元组的页面。
		 */
		// 调用BTreeFile的getEmptyPage，新建一个叶子page（该page在原来page的右侧）
		BTreeLeafPage newPage = ((BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF));

		//获取该page的反向迭代器，从后往前找tuple
		Iterator<Tuple> it = page.reverseIterator();


		//拆分---------------------------------------------------------------------------------
		//获取该叶子page的元组数，用于分两半；取一个元组的数组，用于存放新节点的tuples
		final int nRight = (page.getNumTuples() + 1) / 2;
		Tuple[] rightTuples = new Tuple[nRight];
		//先用一个tuple数组存放要分出来的元组们
		for (int i = 0; i < nRight; ++i) {
			assert it.hasNext();
			rightTuples[nRight - i - 1] = it.next();
		}
		// 再将元组们都从原page删除并放到新page
		for (int i = 0; i < nRight; ++i) {
			//从当前page删除元组
			page.deleteTuple(rightTuples[i]);
			//再把他插入到新的page里
			newPage.insertTuple(rightTuples[i]);
		}

		//将中间节点复制到父节点---------------------------------------------------------------------------
		//获取将被复制到父节点的中间entry（此时迭代器it迭代到的刚好是中间值）
		assert it.hasNext();
		Field midEntryKey = it.next().getField(keyField);

		// 找到父节点，把新entry插进去
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midEntryKey);
		parentPage.insertEntry(new BTreeEntry(midEntryKey, page.pid, newPage.pid));


		//更新同级指针---------------------------------------------------------------------------
		// 检查原page是否有右page，如果有就更新原右节点的指针
		if (page.getRightSiblingId() != null) {
			//取出原来的右节点
			BTreeLeafPage rightSib = ((BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE));
			//原右节点设置新的左节点=新插入的节点
			rightSib.setLeftSiblingId(newPage.pid);
			//更新脏页（原右节点）
			dirtypages.put(rightSib.pid, rightSib);
		}
		//新插入的节点设置其左为原节点
		newPage.setLeftSiblingId(page.pid);
		//新插入的节点设置其右为原右节点（此处可以为空也可以不为空，因此不放入上面的if）
		...
		//更新父节点指针---------------------------------------------------------------------------
		updateParentPointer(tid, dirtypages, parentPage.pid, page.pid);
		updateParentPointer(tid, dirtypages, parentPage.pid, newPage.pid);


		//返回应该插入具有给定键字段的元组的页面-------------------------------------------------------
		return field.compare(Op.LESS_THAN_OR_EQ, midEntryKey) ? page : newPage;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// 调用BTreeFile的getEmptyPage，新建一个中间节点page
		...
		//拆分---------------------------------------------------------------------------------
		//获取该叶子page的entry数，用于分两半；取一个entry的数组，用于存放新节点的entrys（这里与叶节点不同，叶节点是元组，这里是entry）
		final int nRight = (page.getNumEntries()) / 2;
		BTreeEntry[] rightEntries = new BTreeEntry[nRight];
		//先用一个entry数组存放要分出来的entry们
		for (int i = 0; i < nRight; ++i) {
			assert rIt.hasNext();
			rightEntries[nRight - i - 1] = rIt.next();
		}
		// 再将entry们都从原page删除并放到新page
		for (int i = 0; i < nRight; ++i) {
			page.deleteKeyAndRightChild(rightEntries[i]);
			newPage.insertEntry(rightEntries[i]);
		}
		//更新新page的父节点
		...
		//获取将被复制到父节点的中间entry（此时迭代器it迭代到的刚好是中间值）
		assert rIt.hasNext();
		BTreeEntry midEntry = rIt.next();
		// 从页面中删除指定的entry（key + 右子指针）
		page.deleteKeyAndRightChild(midEntry);

		//更新midentry的左右孩子
		midEntry.setLeftChild(page.pid);
		midEntry.setRightChild(newPage.pid);

		// 找到父节点，把midentry插进去
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midEntry.getKey());
		parentPage.insertEntry(midEntry);

		//更新左右page的孩子指针，（前面有大量的entry插入和移除）
		updateParentPointer(tid, dirtypages, parentPage.pid, page.pid);
		updateParentPointer(tid, dirtypages, parentPage.pid, newPage.pid);
		...
		//根据field去决定返回哪个页面
		return field.compare(Op.LESS_THAN_OR_EQ, midEntry.getKey()) ? page : newPage;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//该节点的元组数目
		int myNums = page.getNumTuples();
		//它兄弟节点的元组数目
		int siblingNums = sibling.getNumTuples();
		assert myNums + 1 < siblingNums;
		//计算要被移动的元组数量
		final int moveNums = (myNums + siblingNums) / 2 - myNums;
		assert moveNums >= 1;

		Tuple[] tuples = new Tuple[moveNums];

		//如果该节点的兄弟是右节点（isRightSibling），则从右兄弟steal，迭代器取正向的
		//否则从左兄弟steal，迭代器取反向的
		...
		if (isRightSibling) {
			...
			entry.setKey(sibling.reverseIterator().next().getField(keyField));
		}
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// 自己是右page，兄弟是左page，分别获取自己和兄弟节点的entry数目
		int rightNums = page.getNumEntries();
		int leftNums = leftSibling.getNumEntries();
		assert rightNums + 1 < leftNums;
		// 计算要移动的entry数目
		final int moveNums = (rightNums + leftNums) / 2 - rightNums;
		assert moveNums >= 1;
		//取一个数组来暂存要被移动的entry们
		BTreeEntry[] entries = new BTreeEntry[moveNums];

		//由于是从左兄弟steal，所以取一个反向的迭代器
		...
		for (int i = 0; i < moveNums; ++i) {
			...
			//要被移动的entry的key
			Field leftEntryKey = entries[i].getKey();
			// 把该entry从左兄弟节点移走
			leftSibling.deleteKeyAndRightChild(entries[i]);

			//把steal过来的节点的key设为父entry的key，相当于父节点entry下移，左兄弟entry上移
			entries[i].setKey(parentEntryKey);
			//entry从左边被steal到右边后，原来的右孩子变成左孩子，新的左孩子是原右节点（this page）的第一个最左侧的孩子
			entries[i].setLeftChild(entries[i].getRightChild());
			entries[i].setRightChild(page.iterator().next().getLeftChild());

			//将从左边steal的该entry插入到（右侧）this page
			page.insertEntry(entries[i]);
			//更新已移动页面的父指针
			updateParentPointer(tid, dirtypages, page.pid, entries[i].getLeftChild());
			//更新父节点中指向这两个叶子的entry的key，相当于将原父节点的该entry下移，原左兄弟entry上移
			parentEntry.setKey(leftEntryKey);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// 自己是左page，兄弟是右page，分别获取自己和兄弟节点的entry数目
		int leftNums = page.getNumEntries();
		int rightNums = rightSibling.getNumEntries();
		assert leftNums + 1 < rightNums;
		// 计算要移动的entry数目
		final int moveNums = (leftNums + rightNums) / 2 - leftNums;
		assert moveNums >= 1;
		//取一个数组来暂存要被移动的entry们
		BTreeEntry[] entries = new BTreeEntry[moveNums];

		//由于是从右兄弟steal，所以取一个正向的迭代器
		...
		for (int i = 0; i < moveNums; ++i) {
			...
			//把steal过来的节点的key设为父entry的key，相当于父节点entry下移，左兄弟entry上移
			entries[i].setKey(parentEntryKey);
			//entry从右边被steal到左边后，原来的左孩子变成右孩子，新的右孩子是原左节点（this page）的第一个最右侧的孩子
			entries[i].setRightChild(entries[i].getLeftChild());
			entries[i].setLeftChild(page.reverseIterator().next().getRightChild());

			//将从右边steal的该entry插入到（左侧）this page
			page.insertEntry(entries[i]);
			//更新已移动页面的父指针
			updateParentPointer(tid, dirtypages, page.pid, entries[i].getRightChild());
			//更新父节点中指向这两个叶子的entry的key，相当于将原父节点的该entry下移，原右兄弟entry上移
			parentEntry.setKey(rightEntryKey);
			parent.updateEntry(parentEntry);
		}
		...
		dirtypages.put(parent.pid, parent);
		dirtypages.put(page.pid, page);
		dirtypages.put(rightSibling.pid, rightSibling);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//删除右节点，统一合并到左节点
		//获取要移动的右节点元组数
		final int moveNums = rightPage.getNumTuples();
		//取一个数组暂存要移动的元组们
		Tuple[] tuples = new Tuple[moveNums];
		//获取原右节点的正向迭代器，迭代右节点的元组们
		...
		if (rightPage.getRightSiblingId() != null) {
			...
			//让原右兄弟的左指针指向合并后的节点
			rightSib.setLeftSiblingId(leftPage.pid);
		}
		//让合并后节点的右指针指向原右节点的右兄弟
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//删除右节点，统一合并到左节点
		//获取要移动的右节点entry数
		final int moveNums = rightPage.getNumEntries();
		//取一个数组暂存要移动的entry们
		BTreeEntry[] entries = new BTreeEntry[moveNums];
		//获取原右节点的正向迭代器，迭代右节点的entry们
		...
		//与leaf的merge不同的是，leaf存的是真正的数据，而中间节点存的只是索引entry
		//因此，对中间节点merge时，要把父节点的entry拉下来，相当于最后的节点=左+父entry+右
		//拉下来的父entry要更改其左右孩子，分别为原左节点的最右孩子和原右节点的最左孩子
		...
		//合并节点（从右节点删除，从左节点插入）
		for (int i = 0; i < moveNums; ++i) {
			rightPage.deleteKeyAndLeftChild(entries[i]);
			leftPage.insertEntry(entries[i]);
		}
		//更新父节点指针
...
//		}
	...
	/**
	 * BTreeReverseSearchIterator and BTreeFileReverseIterator are modified on the basis of
	 * BTreeFileIterator and BTreeSearchIterator
	 */

	/**
	 * Helper class that implements the Java Iterator for tuples on a BTreeFile (Reverse scan)
	 */
